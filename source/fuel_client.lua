...
AddEventHandler('fuel:refuelFromPump', function(pumpObject, ped, vehicle)
    ...
    while isFueling do
        ...
        if pumpObject then
            ...
            local extraString = "\n" .. Config.Lang.TotalCost .. ": ~g~$" .. Round(todaycost, 1)
			
            DrawText3Ds(stringCoords.x, stringCoords.y, stringCoords.z + 1.2, Config.Lang.CancelFuelingPump .. extraString)
            ...
            DrawText3Ds(vehicleCoords.x, vehicleCoords.y, vehicleCoords.z + 0.5, Config.Lang.CancelFuelingJerryCan .. "\nGas can: ~g~" .. Round(GetAmmoInPedWeapon(ped, 883325847) / 4500 * 100, 1) .. "% | Vehicle: " .. Round(currentFuel, 1) .. "%")
        end

        if not animPlaying or not IsEntityPlayingAnim(ped, "timetable@gardener@filling_can", "gar_ig_5_filling_can", 3) then
            TaskPlayAnim(ped, "timetable@gardener@filling_can", "gar_ig_5_filling_can", 2.0, 8.0, -1, 50, 0, false, false, false)
            animPlaying = true
        end
		print(json.encode(Config.Lang.CancelFuelingJerryCan))
	...
	while true do
		...
		if not isFueling and ((isNearPump and GetEntityHealth(isNearPump) > 0) or (GetSelectedPedWeapon(ped) == 883325847 and not isNearPump)) then
			if IsPedInAnyVehicle(ped, false) and GetPedInVehicleSeat(GetVehiclePedIsIn(ped, false), -1) == ped then
				...
				DrawText3Ds(pumpCoords.x, pumpCoords.y, pumpCoords.z + 1.2, Config.Lang.ExitVehicle)
			else
					...
					if not DoesEntityExist(GetPedInVehicleSeat(vehicle, -1)) then
						...
						if GetVehicleFuelLevel(vehicle) < 95 and canFuel then
							if currentCash > 0 then
								DrawText3Ds(stringCoords.x, stringCoords.y, stringCoords.z + 1.2, Config.Lang.EToRefuel)
								...
								DrawText3Ds(stringCoords.x, stringCoords.y, stringCoords.z + 1.2, Config.Lang.NotEnoughCash)
							end
						elseif not canFuel then
							DrawText3Ds(stringCoords.x, stringCoords.y, stringCoords.z + 1.2, Config.Lang.JerryCanEmpty)
						else
							DrawText3Ds(stringCoords.x, stringCoords.y, stringCoords.z + 1.2, Config.Lang.FullTank)
						end
					end
				elseif isNearPump then
					...
					if currentCash >= Config.JerryCanCost then
						if not HasPedGotWeapon(ped, 883325847, false) then
							DrawText3Ds(stringCoords.x, stringCoords.y, stringCoords.z + 1.2, Config.Lang.PurchaseJerryCan)

							if IsControlJustReleased(0, 38) then
							
								TriggerServerEvent('fuel:addPetrolCan')
							
								local weapon = exports.ox_inventory:Search("count", "money", nil)
								if weapon >= Config.PurchaseJerryCan then
									print(weapon)

								local money = exports.ox_inventory:Search("count", "money", nil)
								if money >= Config.JerryCanCost then
									print(money)
							...
							if refillCost > 0 then
								if currentCash >= refillCost then
									DrawText3Ds(stringCoords.x, stringCoords.y, stringCoords.z + 1.2, Config.Lang.RefillJerryCan .. refillCost)
									...
									DrawText3Ds(stringCoords.x, stringCoords.y, stringCoords.z + 1.2, Config.Lang.NotEnoughCashJerryCan)
								end
							else
								DrawText3Ds(stringCoords.x, stringCoords.y, stringCoords.z + 1.2, Config.Lang.JerryCanFull)
							end
						end
					else
						DrawText3Ds(stringCoords.x, stringCoords.y, stringCoords.z + 1.2, Config.Lang.NotEnoughCash)
